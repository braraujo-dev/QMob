...
class _QueuePageState extends State<QueuePage> {
            ...
            if (state is QueueErrorState) {
              return Center(
                child: Column(
                  ...
                  children: [
                    Text('Erro ao carregar fila', style: TextStyle(color: Colors.white)),
                    TextButton(onPressed: () => _controller.fetchQueue(), child: Text('Tentar novamente')),
                  ],
                ),
              );
            }
  ...
  Widget _buildQueueList(QueueLoadedState state) {
    return SingleChildScrollView(
      physics: const AlwaysScrollableScrollPhysics(),
      child: Padding(
        padding: const EdgeInsets.all(24.0),
        child: Column(
          ...
          children: [
                ...
                child: Column(
                  children: [
                    ...
                    ElevatedButton.icon(
                      ...
                      label: const Text('Sair da Fila (Checkout)', style: TextStyle(fontWeight: FontWeight.bold)),
                    ),
                  ],
                ),
              ),
            ...
            const Center(
              child: Column(
                children: [
                  Text('Deslize para ver mais motoristas', style: TextStyle(color: AppColors.slate500, fontSize: 12)),
                  Icon(Icons.keyboard_arrow_down, color: AppColors.slate500),
                ],
              ),
            ),
          ],
  ...
  void _showCheckoutDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        title: const Text('Sair da Fila?', style: TextStyle(color: Colors.white)),
        content: const Text('Você perderá sua posição atual na fila de espera.', style: TextStyle(color: AppColors.slate400)),
        actions: [
          TextButton(
          ...
          TextButton(
            ...
            child: const Text('Sair da Fila', style: TextStyle(color: Colors.redAccent, fontWeight: FontWeight.bold)),
          ),
        ],
      ),
    );
